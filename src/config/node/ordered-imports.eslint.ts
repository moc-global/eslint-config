...
import { resolveTsconfigPath, resolveTsconfigPaths } from '../../core/tsconfig-utils.js';
 ...
 * @param options - Optional project root and tsconfig path used to derive internal import groups.
 ...
 * @returns The simple-import-sort flat-config array with tsconfig-derived groups.
 */
export function createOrderedImportsConfig(options: { rootDir?: string; tsconfig?: string } = {}) {
  const tsconfigPath = resolveTsconfigPath(options);
  const allPaths = resolveTsconfigPaths(tsconfigPath);

  let tsconfigPathsGroups: string[][] = [];
